/*
  ...
  Servicios expuestos:
    ...
    tradeInCatalogService -> identidad de modelos de CANJE (Paso 1, fijo)
    valuationService -> valor de canje por modelo+capacidad (Admin → Valoración)
...
function normalizeStore(raw) {
  ...
  // Migración automática: si no hay valoraciones guardadas todavía
  // (instalación nueva, o una versión anterior que no tenía este
  // módulo), se cargan las de fábrica — que reproducen EXACTO lo que
  // antes calculaba la fórmula vieja (precioBaseUSD × factor de
  // capacidad), así no se pierde ningún dato en la migración.
...
/* ============================================================
   tradeInCatalogService — IDENTIDAD del catálogo de CANJE (Paso 1).
   ------------------------------------------------------------
   ...
   quede vacía o dependa de ediciones del catálogo de venta NI de
   Valoración. Solo expone id + nombre: el VALOR de cada modelo
   (por capacidad) lo administra valuationService, acá abajo.
   ============================================================ */
...
/* ============================================================
   ...
   independiente y 100% editable. Esto es lo que reemplaza a la
   vieja fórmula "precioBaseUSD del modelo × factor de la
   capacidad" (que ya no existe en ningún lado del código).
   Persistido en el STORE bajo "valoraciones".
   ============================================================ */
const valuationService = {
  ...
  create(data) {
    ...
    const capacidad = STATIC_CONFIG.capacidades.find((c) => c.id === capacidadId);
    const item = {
      ...
      valorUSD: Math.max(0, Number(data.valorUSD) || 0),
    };
  ...
  update(id, patch) {
    ...
    if (!item) return null;
    if (patch.valorUSD !== undefined) item.valorUSD = Math.max(0, Number(patch.valorUSD) || 0);
    StorageService.saveStore(store);
    return item;
  },
  remove(id) {
...
const pricingService = {
  ...
  calculateQuote({ modeloId, capacidadId, bateriaId, desperfectoIds }) {
    // La IDENTIDAD del modelo/capacidad viene siempre de las listas
    // fijas (independientes de Admin); el VALOR viene siempre de
    // valuationService (Admin → Valoración) — nunca de un factor.
    const modelo = tradeInCatalogService.getById(modeloId);
